...
with st.expander("How it learns from rate sheets", expanded=False):
    st.markdown(
        ...
        - When you upload a new sheet, SmartDesk uses **those rules** for the rest of your session.  
        - You’ll see the **Top 5** matching lenders and a best pick with a short reason.  
        """
    )
...
with left:
    ...
    with st.form("deal_form"):
        ...
        with col3:
            ...
            gig_income = st.number_input(
                "Gig Income ($/month)", 0, 20000, 0, 50, disabled=not gig_flag
            )
...
with right:
    ...
    if rs_file is not None:
        try:
            ...
            st.success(
                f"Loaded {len(st.session_state['rate_rules'])} lender rows from **{rs_file.name}**."
            )
...
def score_lender(row, features):
    """
    Return (eligible: bool, reason: str, score: float)
    for a lender row based on deal 'features'.
    """
...
if submitted:
    ...
    with cols[1]:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown('<div class="metric">Top 5 Matches</div>', unsafe_allow_html=True)
        if len(top) > 0:
            st.dataframe(
